...
describe Toggle::Backend::Redis do
  describe "Toggle Behaviour" do
    it "returns true when feature is enabled" do
      subject = Toggle::Backend::Redis.new(redis: TestingRedis.current)
  ...
  describe "#features" do
    it "returns all features" do
      ...
      subject.enable("payment_methods::pix")
  ...
  describe "#clear" do
    it "clears the storage" do
      ...
      subject.enabled?("store::new_checkout").should be_true
      subject.enabled?("payment_methods::pix").should be_true

      subject.clear

      subject.enabled?("store::new_checkout").should be_false
      subject.enabled?("payment_methods::pix").should be_false
    end
  end
end
